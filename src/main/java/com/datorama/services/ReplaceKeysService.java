...
public class ReplaceKeysService {
	...
	public static ReplaceKeysService getInstance() {
		if (replaceKeysService == null) {
			synchronized (ReplaceKeysService.class) {
				if (replaceKeysService == null) {
					replaceKeysService = new ReplaceKeysService();
				}
			}
		}
		return replaceKeysService;
	}
